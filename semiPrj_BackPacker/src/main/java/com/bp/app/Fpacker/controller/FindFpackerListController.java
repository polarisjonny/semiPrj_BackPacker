...
@WebServlet ("/findFpacker/list")
public class FindFpackerListController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if(searchType ==null || searchType.equals("")) {
				bvoList = gbs.getList(2, pvo);
			}else {
				bvoList = gbs.getList(2 ,pvo, searchType, searchValue);		
			}
			...
			req.getRequestDispatcher("/WEB-INF/views/FpackerBoard/findFpackerList.jsp").forward(req, resp); 
		} catch (Exception e) {
