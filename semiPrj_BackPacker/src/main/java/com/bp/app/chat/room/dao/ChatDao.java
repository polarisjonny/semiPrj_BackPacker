...
public class ChatDao {
	...
	public List<ChattingRoomVo> openRoomList1(Connection conn, ChattingRoomVo crv, PageVo pv, String searchType, String searchValue) throws Exception {
		String sql = "";
	      ...
	      if("CHATTINGUSERNICK".equals(searchType)) {
	         //SQL (1번유저)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE, S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE  CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND NOT (S.CHATTING_STATUS >10) AND (CHATTINGUSERNICK LIKE '%'||?||'%' OR CHATTINGUSER2NICK LIKE '%'||?||'%')ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDTITLE".equals(searchType)) {
	         //SQL (게시판제목 검색)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE  CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND NOT (S.CHATTING_STATUS >10)  AND (BOARDTITLE LIKE '%'||?||'%' OR BOARDTITLE LIKE '%'||?||'%') ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDCATEGORYNAME".equals(searchType)) {
		     //SQL (게시판제목 타입)
		     sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND NOT (S.CHATTING_STATUS >10)  AND  (BOARDCATEGORYNAME LIKE '%'||?||'%' OR  BOARDCATEGORYNAME LIKE '%'||?||'%' ) ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      } else {
	         //값이 이상함 => 기본 목록 조회
