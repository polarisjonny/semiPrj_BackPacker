...
public class ChatDao {
	...
	public int selectCnt2(Connection conn, String searchType, String searchValue, String no) throws Exception {
		String sql = "SELECT COUNT(*) FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE, B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE,  BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON  C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO  JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO ) S WHERE (S.CHATTING_USER2_NO = ? OR S.CHATTING_USER_NO=?) AND NOT ( S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y' ) AND S.CHATTING_STATUS < 3 AND NOT S.MATCHING_CHECK2  = 'O'";
	...
	public List<ChattingRoomVo> openRoomList1(Connection conn, ChattingRoomVo crv, PageVo pv) throws Exception {
         ...
         JDBCTemplate.close(rs);
         JDBCTemplate.close(pstmt);
         List<ChattingRoomVo> roomList3 = openRoomList2(conn, crv, pv, roomList);
         return roomList3;
	...
	public List<ChattingRoomVo> openRoomList2(Connection conn, ChattingRoomVo crv, PageVo pv, List<ChattingRoomVo> roomList) throws Exception {
		 String sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK, S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE, S.BOARDCATEGORYNAME,S.CHATTINGUSERID, S.CHATTINGUSER2ID FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE, B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID,MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO ) S WHERE CHATTING_USER2_NO = ?  AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y') AND  S.CHATTING_STATUS =1 AND S.MATCHING_CHECK2 = 'N' ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
        PreparedStatement pstmt = conn.prepareStatement(sql);
        pstmt.setString(1, crv.getChattingUserNo());
        pstmt.setInt(2, pv.getBeginRow());
        pstmt.setInt(3, pv.getLastRow());
        ...
        List<ChattingRoomVo> roomList1 = roomList;
        while(rs.next()) {
           String chattingRoomNo = rs.getString("CHATTING_ROOM_NO");
           String chattingUserNo = rs.getString("CHATTING_USER_NO");
           String chattingUser2No = rs.getString("CHATTING_USER2_NO");
	...
	public List<ChattingRoomVo> openRoomList1(Connection conn, ChattingRoomVo crv, PageVo pv, String searchType, String searchValue) throws Exception {
		String sql = "";
	      ...
	      if("CHATTINGUSERNICK".equals(searchType)) {
	         //SQL (1번유저)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE, S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE  CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y') AND CHATTING_STATUS =1 AND S.MATCHING_CHECK = 'N'AND (CHATTINGUSERNICK LIKE '%'||?||'%' OR CHATTINGUSER2NICK LIKE '%'||?||'%')ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDTITLE".equals(searchType)) {
	         //SQL (게시판제목 검색)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE  CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y') AND CHATTING_STATUS =1 AND  S.MATCHING_CHECK= 'N' AND (BOARDTITLE LIKE '%'||?||'%' OR BOARDTITLE LIKE '%'||?||'%') ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDCATEGORYNAME".equals(searchType)) {
		     //SQL (게시판제목 타입)
		     sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE CHATTING_USER_NO=? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y') AND CHATTING_STATUS =1 AND S.MATCHING_CHECK = 'N' AND  (BOARDCATEGORYNAME LIKE '%'||?||'%' OR  BOARDCATEGORYNAME LIKE '%'||?||'%' ) ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      } else {
	         //값이 이상함 => 기본 목록 조회
	      ...
	      List<ChattingRoomVo> roomList = new ArrayList<>();
	         ...
	         JDBCTemplate.close(rs);
	         JDBCTemplate.close(pstmt);
	         List<ChattingRoomVo> roomList3 = openRoomList2(conn, crv, pv, searchType,searchValue, roomList);
	         return roomList3;
	...
	public List<ChattingRoomVo> openRoomList2(Connection conn, ChattingRoomVo crv, PageVo pv, String searchType, String searchValue,List<ChattingRoomVo> roomList) throws Exception {
		String sql = "";
	      ...
	      if("CHATTINGUSERNICK".equals(searchType)) {
	         //SQL (1번유저)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE, S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE CHATTING_USER2_NO = ? AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND CHATTING_STATUS =1 AND S.MATCHING_CHECK2 = 'N' AND (CHATTINGUSERNICK LIKE '%'||?||'%' OR CHATTINGUSER2NICK LIKE '%'||?||'%')ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDTITLE".equals(searchType)) {
	         //SQL (게시판제목 검색)
	         sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE CHATTING_USER2_NO = ?  AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND CHATTING_STATUS =1 AND S.MATCHING_CHECK2 = 'N' AND  (BOARDTITLE LIKE '%'||?||'%' OR BOARDTITLE LIKE '%'||?||'%') ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      }else if("BOARDCATEGORYNAME".equals(searchType)) {
		     //SQL (게시판제목 타입)
		     sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT S.CHATTING_ROOM_NO, S.CHATTING_USER_NO, S.CHATTING_USER2_NO, S.GUIDE_BOARD_NO, S.MATCHING_CHECK, S.MATCHING_CHECK2, S.CHATTING_STATUS, S.BOARDTITLE, S.CHATTINGUSERNICK, S.CHATTINGUSER2NICK ,S.BOARDIMG, S.CHATTINGUSER2PROFILE, S.CHATTINGUSERPROFILE , S.BOARDCATEGORYNAME, S.CHATTINGUSERID, S.CHATTINGUSER2ID  FROM ( SELECT C.CHATTING_ROOM_NO, C.CHATTING_USER_NO, C.CHATTING_USER2_NO, C.GUIDE_BOARD_NO, C.MATCHING_CHECK, C.MATCHING_CHECK2, C.CHATTING_STATUS, B.TITLE AS BOARDTITLE,B.MAIN_IMG AS BOARDIMG, M.NICK AS CHATTINGUSERNICK, M.PROFILE_IMAGE AS CHATTINGUSERPROFILE, M.ID AS CHATTINGUSERID, MT.ID AS CHATTINGUSER2ID, MT.NICK AS CHATTINGUSER2NICK, MT.PROFILE_IMAGE AS CHATTINGUSER2PROFILE, BC.CATEGORY_NAME AS BOARDCATEGORYNAME FROM CHATTING_ROOM C JOIN MEMBER M ON C.CHATTING_USER_NO = M.MEMBER_NO JOIN MEMBER MT ON C.CHATTING_USER2_NO = MT.MEMBER_NO JOIN GUIDE_BOARD B ON C.GUIDE_BOARD_NO = B.GUIDE_BOARD_NO JOIN GUIDE_BOARD_CATEGORY BC ON B.GUIDE_BOARD_CATEGORY_NO = BC.CATEGORY_NO) S WHERE CHATTING_USER2_NO = ?  AND NOT (S.MATCHING_CHECK = 'Y' AND S.MATCHING_CHECK2 = 'Y')  AND CHATTING_STATUS =1 AND S.MATCHING_CHECK2 = 'N' AND  (BOARDCATEGORYNAME LIKE '%'||?||'%' OR  BOARDCATEGORYNAME LIKE '%'||?||'%' ) ORDER BY CHATTING_ROOM_NO DESC ) T ) WHERE RNUM BETWEEN ? AND ?";
	      } else {
	         //값이 이상함 => 기본 목록 조회
	         return openRoomList1(conn, crv, pv);
	      }
	      ...
	      //tx || rs
	      List<ChattingRoomVo> roomList2= roomList;
	         while(rs.next()) {
	            String chattingRoomNo = rs.getString("CHATTING_ROOM_NO");
	            String chattingUserNo = rs.getString("CHATTING_USER_NO");
	            String chattingUser2No = rs.getString("CHATTING_USER2_NO");
	...
	public int outChatRoom(Connection conn, ChattingRoomVo vo) throws Exception {
		String sql = "UPDATE CHATTING_ROOM SET MATCHING_CHECK2 = 'O', CHATTING_STATUS = CHATTING_STATUS + 1 WHERE MATCHING_CHECK2 = 'N' AND CHATTING_ROOM_NO = ? AND CHATTING_USER2_NO = ? AND CHATTING_STATUS <3";
	...
	public int outMyBoardChatRoom(Connection conn, ChattingRoomVo vo) throws Exception {
		String sql = "UPDATE CHATTING_ROOM SET MATCHING_CHECK = 'O', CHATTING_STATUS = CHATTING_STATUS + 1 WHERE MATCHING_CHECK = 'N' AND CHATTING_ROOM_NO = ? AND CHATTING_USER_NO = ? AND  CHATTING_STATUS <3";
	...
	public int submitMyBoardChatRoom(Connection conn, ChattingRoomVo vo) throws Exception {
		String sql = "UPDATE CHATTING_ROOM SET MATCHING_CHECK = 'Y' WHERE CHATTING_ROOM_NO = ?  AND MATCHING_CHECK = 'N' AND CHATTING_USER_NO = ? AND  CHATTING_STATUS <3";
	...
	public int submitChatRoom(Connection conn, ChattingRoomVo vo) throws Exception {
		String sql = "UPDATE CHATTING_ROOM SET MATCHING_CHECK2 = 'Y' WHERE CHATTING_ROOM_NO = ? AND MATCHING_CHECK2 = 'N' AND CHATTING_USER2_NO = ? AND CHATTING_STATUS <3";
