...
public class MyPageMatchingMemberDao {
	...
	public List<MemberVo> selectMatchingMemberListForPage(Connection conn, PageVo pv, String memberNo) throws Exception {
		String sql = "SELECT M.MEMBER_NO AS MEMBER_NO, M.NICK AS NICK, M.PROFILE_IMAGE AS PROFILE_IMAGE FROM ( SELECT ROWNUM AS RNUM, T.* FROM ( SELECT * FROM CHATTING_ROOM WHERE (CHATTING_USER_NO = ? OR CHATTING_USER2_NO = ?) AND MATCHING_CHECK = 'Y' AND MATCHING_CHECK2 = 'Y' AND CHATTING_STATUS = '1'  ) T ) C JOIN MEMBER M ON ( (M.MEMBER_NO = C.CHATTING_USER_NO OR M.MEMBER_NO = C.CHATTING_USER2_NO) AND M.MEMBER_NO <> ?) WHERE RNUM BETWEEN ? AND ? ORDER BY CHATTING_ROOM_NO DESC";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, memberNo);
		pstmt.setString(2,  memberNo);
		pstmt.setString(3,  memberNo);
