...
public class BoardManagerDao {
	...
	public int getAccompanyBoardListCnt(Connection conn, String searchType, String searchValue) throws Exception {
		String sql = "SELECT COUNT(*) FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT AS REPORTCNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO) WHERE DELETE_YN = 'N' AND GUIDE_BOARD_CATEGORY_NO = 3 ";
	...
	public List<AccompanyBoardVo> getAccompanyBoardList(Connection conn, PageVo pv) throws Exception {
		...
		String sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO WHERE DELETE_YN = 'N' ORDER BY GUIDE_BOARD_NO DESC ) T ) WHERE GUIDE_BOARD_CATEGORY_NO = 3 AND RNUM BETWEEN ? AND ?";
	...
	public List<AccompanyBoardVo> getAccompanyBoardList(Connection conn, PageVo pv, String searchType,String searchValue) throws Exception {
		...
		if("title".equals(searchType)) {
			//sql(제목으로검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO WHERE DELETE_YN = 'N' AND B.TITLE LIKE ('%'||?||'%') ORDER BY  GUIDE_BOARD_NO DESC ) T ) WHERE GUIDE_BOARD_CATEGORY_NO = 3 AND RNUM BETWEEN ? AND ?";
			
		}else if("writerNick".equals(searchType)) {
			//sql(작성자로검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO WHERE DELETE_YN = 'N' AND M.NICK LIKE ('%'||?||'%') ORDER BY GUIDE_BOARD_NO DESC ) T ) WHERE GUIDE_BOARD_CATEGORY_NO = 3 AND  RNUM BETWEEN ? AND ?";
			
		}else if ("writerId".equals(searchType)) {
			//sql(카테고리로검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO WHERE DELETE_YN = 'N' AND M.ID LIKE ('%'||?||'%') ORDER BY GUIDE_BOARD_NO DESC ) T ) WHERE GUIDE_BOARD_CATEGORY_NO = 3 AND  RNUM BETWEEN ? AND ?";
			
		}else if ("reportCnt".equals(searchType)) {
			//sql(카테고리로검색)
			sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM ( SELECT B.GUIDE_BOARD_NO, B.WRITER_NO, B.GUIDE_BOARD_CATEGORY_NO, B.TITLE, B.CONTENT, B.ENROLL_DATE, B.MODIFY_DATE, B.HIT, B.MATCHING_STATE, B.TRAVEL_EXPENSE, B.DELETE_YN, B.REPORT_CNT, M.NICK AS WRITERNICK, M.ID AS WRITERID FROM GUIDE_BOARD B JOIN MEMBER M ON B.WRITER_NO = M.MEMBER_NO WHERE DELETE_YN = 'N' AND REPORT_CNT >= ? ORDER BY GUIDE_BOARD_NO DESC ) T ) WHERE GUIDE_BOARD_CATEGORY_NO =3 AND  RNUM BETWEEN ? AND ?";
		}else {
