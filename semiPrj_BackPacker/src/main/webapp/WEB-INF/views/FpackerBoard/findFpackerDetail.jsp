...
<body>
	<div id="wrap">
		...
		<main>
 	        <img class="middle-img" src="${root}/static/img/Fpacker/${gbvo.mainImg}" alt="">
			<div id="main-box">
				<div id="blank1"></div>
				<div id="main-area">
						 ...
						 <div id="info-box">
							...
							<div id="id-area">
								...
								<div id="btn-area">
									...
									<button class="s-btn btn-red" onclick="location.href='${root}/findFpacker/del?no=${gbvo.guideBoardNo}'">삭제</button>
									<script>
...
<script>
		...
		function writeComment(){
			...
			$.ajax({
				url : "${root}/findFpacker/reply/write",
				type : "POST",
				data : {
					findFpackerNo : '${gbvo.guideBoardNo}',
					content : comment ,
				},
				success: (x)=>{
					console.log(x);
		...
		function delComment(guideReplyNo){
			$.ajax({
				url: "${root}/findFpacker/reply/delete",
				type: "post",
				data : {
		...
		function loadComment(){
			$.ajax({
				url : '${root}/findFpacker/reply/list',
				type: "GET" ,
				data : {
					findFpackerNo : '${gbvo.guideBoardNo}'
				},
		...
		reportBtn.addEventListener('click',function(){
			...
			const left = (screen.width/2)-(width/2);
			const top = 0;
			window.open('${root}/findFpacker/report?boardNo='+no,'', 'width=' + width + ', height=' + height + ', left=' + left + ', top=' + top);
		});
