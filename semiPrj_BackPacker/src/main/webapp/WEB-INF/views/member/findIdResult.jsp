...
<body>
	...
	<main>
		<div id="wrap">
			...
			<c:if test="${not empty tempMember}">
				<span>${tempMember.nick}님의 아이디는 ${tempMember.id}입니다.</span>
			</c:if>
		...
		<div id="button-area">
			...
			<button class="button-style" id="password-find-btn">비밀번호 찾기</button>
			<c:if test="${empty tempMember}">
	...
	<script>
		...
		const passwordFindBtn = document.querySelector("#password-find-btn");
		...
		passwordFindBtn.addEventListener("click", function(){
			location.href = "${root}/member/find/password";
		});
